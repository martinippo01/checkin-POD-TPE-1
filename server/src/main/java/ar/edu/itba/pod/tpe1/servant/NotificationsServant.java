...
import ar.edu.itba.pod.tpe1.protos.NotificationsService.*;
...
public class NotificationsServant extends NotificationsServiceGrpc.NotificationsServiceImplBase {
    ...
    public void registerNotifications(
            RegisterNotificationsRequest req,
            StreamObserver<RegisterNotificationsResponse> responseObserver
    ) {
        try {
            ...
            responseObserver.onNext(buildNotificationProto(new Notification.Builder().setNotificationType(NotificationType.SUCCESSFUL_REGISTER).build()));
    ...
    public void removeNotifications(
            RemoveNotificationsRequest req,
            StreamObserver<RemoveNotificationsResponse> responseObserver
    ) {
        Airline airline = new Airline(req.getAirline());
        ...
        responseObserver.onNext(RemoveNotificationsResponse.newBuilder().build());
        responseObserver.onCompleted();
        logger.info("Airline: {} successfully unregistered to notifications service", airline.getName());
    }

    private RegisterNotificationsResponse buildNotificationProto(Notification notification) {
        if (notification == null) return null;
        // TODO: Check notification type, in order to set the respective arguments of the proto messsage
        RegisterNotificationsResponse.Builder builder = RegisterNotificationsResponse.newBuilder();
